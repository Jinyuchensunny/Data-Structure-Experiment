...
void findebook(){
	manage manage8;
	manage8.findAllBooks();;//调用manage类中findAllBooks（）函数
	system("pause");
	
}
void searchAllBorrow() {
	manage manage9;
	manage9.findAllBorrow();//调用reader类中的findAllBooks()函数
	system("pause");
}
...
void getManage()//管理员登陆函数
{
	...
	if(b==1)//登陆成功
	{
		...
		cout<<"4.删除书籍信息"<<endl;
		cout<<"5.修改书籍信息"<<endl;
		cout<<"6.查询所有书籍信息"<<endl;
		cout<<"7.查询所有借阅信息" << endl;
		cout<<"8.退出"<<endl;
		int t;
		cin>>t;
		switch(t)
			...
			case 6:findebook();break;//调用findbook()函数
			case 7:searchAllBorrow(); break;//调用searchAllBorrow()函数
			case 8:
				cout<<"感谢您的使用！"<<endl;//退出
				system("pause");
				exit(0);
...
//读者相关函数
void search(reader &reader3)//查询函数
{
...
void borrowBook(reader* reader4)//借书函数
{
...
void searchAllBook(reader &reader7)//借书函数
{
	//reader reader7;//实例化对象
	reader7.findAllBooks();//调用reader类中的findAllBooks()函数
	system("pause");
}
...
void searchAllBorrow(reader &reader8) {
	reader8.findAllBorrow();//调用reader类中的findAllBooks()函数
	system("pause");
}

void searchBookBorrow(reader &reader9) {
	reader9.findBorrowbyBookId();//调用reader类中的findAllBooks()函数
	system("pause");
}

void returnBook(reader &reader5)//还书函数
{
...
void getStudent()//登录函数
{
	...
	if(a==1)//登陆成功
	{
		...
		cout<<"3.借阅书籍"<<endl;
		cout<<"4.返还书籍"<<endl;
		cout<<"5.查询读者借阅信息" << endl;
		cout<<"6.查询书籍借阅信息" << endl;
		cout<<"7.退出"<<endl;
		int t;
		cin>>t;
		switch(t)
			{
			case 1:search(reader5);break;//调用search()函数
			case 2:searchAllBook(reader5); break;//调用searchAllBook()函数
			case 3:borrowBook(&reader5);break;//调用borrowBook()函数
			case 4:returnBook(reader5);break;//调用returnBook()函数
			case 5:searchAllBorrow(reader5); break;//调用searchAllBorrow()函数
			case 6:searchBookBorrow(reader5);break;//调用searchBookBorrow()函数
			case 7:
				cout<<"感谢您的使用！"<<endl;//退出
				system("pause");
				exit(0);
